...
const SpaceDetailScreen = () => {
    ...
    try {
      await collaborationService.subscribeToSpace(id as string, 'space-root', {
        ...
        onCallStarted: (data) => {
          ...
          // ✅ UPDATE SPACE STATE: Set active_call so the LIVE icon appears in header
          setSpace((prev: any) => ({
            ...prev,
            active_call: data.call || data
          }));
        ...
        onCallEnded: (data) => {
          ...
          // ✅ UPDATE SPACE STATE: Clear active_call so the LIVE icon disappears
          setSpace((prev: any) => ({
            ...prev,
            active_call: null
          }));
  ...
  const handleGuestJoin = async (guestName: string) => {
    try {
      const { user: guestUser, token, space: joinedSpace, participation } = await collaborationService.joinSpaceAsGuest(id as string, guestName);
      ...
      // ✅ AUTO-POPUP: If it's a channel and there's an active call, join it automatically for the guest
      if (joinedSpace.space_type === 'channel' && joinedSpace.active_call) {
        console.log('📺 Auto-joining broadcast for guest:', joinedSpace.active_call.id);
        startCall({
          spaceId: id as string,
          spaceType: 'channel',
          callId: joinedSpace.active_call.id,
          type: joinedSpace.active_call.type || 'video',
        });
      }
      ...
      if (Platform.OS === 'web') {
        if (navigator.share) {
          try {
            ...
            console.log('User cancelled or share failed:', shareError);
          }
        } else {
          ...
          <BackButton
            onPress={() => {
              if (params.returnTo) {
                ...
                // router.back() cannot be used here because every notification
                // push() adds a new space screen on top of the stack.
                router.replace('/(tabs)/chats');
              }
            }}
          />
