...
return [
    ...
    'Cosmology' => [
        ...
        'phase1' => function(&$state, $solver) {
            ...
            foreach ([1, 5, 10, 50, 100, 500, 1000] as $d_Mpc) {
                $v = 67.40000000000001 * $d_Mpc;
                $state['trials'][] = ['Distance d (Mpc)' => $d_Mpc, 'v = H₀·d (km/s)' => number_format($v, 0), '% of c' => number_format($v / 299800.0 * 100, 3) . '%', 'Observable?' => $v < 299800.0 ? '✅ Yes' : '❌ Beyond Hubble horizon'];
            }
        },
        'phase2' => function(&$state, $solver) {
    ...
    'Geology' => [
        'meta' => [
            'inductive_limit' => 'Plate tectonics is measured in real time by GPS to mm/yr precision. Radiometric dating has been cross-validated against astronomical (Cepheid), geological (stratigraphy), and geophysical (paleomagnetic) methods for 80+ years. Earth\'s geological history (4.543 Gyr) is independently constrained by multiple dating systems (U-Pb, Rb-Sr, Sm-Nd, K-Ar). Plate tectonics explains earthquakes, volcanoes, mountain building, and ocean floor morphology — no alternative framework exists.',
        ],
        'phase1' => function(&$state, $solver) {
            ...
            foreach ([0, 1, 2, 3, 4, 4.47] as $t_Gyr) {
                $t_half = 4.47;
                // U-238 half-life in Gyr
                $fraction_remaining = pow(0.5, $t_Gyr / $t_half);
                $state['trials'][] = ['Time t (Gyr)' => $t_Gyr, 'U-238 Remaining' => number_format($fraction_remaining * 100, 1) . '%', 'Pb-206 Produced' => number_format((1 - $fraction_remaining) * 100, 1) . '%', 'N/N₀ = 2^(-t/t½)' => number_format($fraction_remaining, 4)];
            }
        },
        'phase2' => function(&$state, $solver) {
    ...
    'Meteorology' => [
        ...
        'phase1' => function(&$state, $solver) {
            ...
            foreach ([255, 270, 288, 300, 315] as $T_K) {
                $P = 5.67E-8 * pow($T_K, 4);
                $state['trials'][] = ['Temperature T (K)' => $T_K, 'T (°C)' => $T_K - 273, 'P = σT⁴ (W/m²)' => number_format($P, 0), 'Context' => match (true) {
    ...
    'Ecology' => [
        'meta' => [
            'inductive_limit' => 'Lotka-Volterra equations are verified in laboratory populations (Gause 1934) and field observations (Hudson Bay lynx-hare data, Serengeti wildebeest-lion). Logistic growth is confirmed across species from bacteria to elephants. Shannon diversity indices are mathematically grounded information theory. The 10% energy transfer rule is an empirical average across ecosystems — actual range 5-20% depending on ecosystem type.',
        ],
        'phase1' => function(&$state, $solver) {
            ...
            foreach ([10, 50, 100, 200, 500, 900, 999] as $N) {
                $dNdt = $r * $N * (1 - $N / $K);
                $state['trials'][] = ['Population N' => $N, 'N/K ratio' => number_format($N / $K, 3), 'dN/dt (r=0.5)' => number_format($dNdt, 1), 'Growth Phase' => $N < $K / 2 ? 'Accelerating ↑' : ($N < $K * 0.9 ? 'Decelerating ↓' : 'Near K ≈ 0')];
            }
        },
        'phase2' => function(&$state, $solver) {
