...
return [
    'propositional_tautology' => [
        'phase2' => function(array $state, $solver) {
    ...
    'propositional_implication' => [
        ...
        $hasP_minor = $solver->textContains($positiveTexts, $P, false) || $solver->textContains([$thesisWithoutImplication], $P, false);
        // A negative match means it appears in negative texts OR it appears negated in the thesis
        $hasNotP_minor = $solver->textContains($negativeTexts, $P, false) || $solver->textContains([$thesisWithoutImplication], $P, true);
        
        $hasQ_minor = $solver->textContains($positiveTexts, $Q, false) || $solver->textContains([$thesisWithoutImplication], $Q, false);
        $hasNotQ_minor = $solver->textContains($negativeTexts, $Q, false) || $solver->textContains([$thesisWithoutImplication], $Q, true);
    ...
    'disjunctive_syllogism' => [
        ...
        $hasNotA = $solver->textContains($allTexts, $A, true);
        $hasNotB = $solver->textContains($allTexts, $B, true);
    ...
    'hypothetical_syllogism' => [
        ...
        if (!$impl2) {
            ...
            return $solver->solveImplication($state);
        }
    ...
    'constructive_dilemma' => [
        ...
        if (!$impl1) {
            return $solver->solveDisjunctive($state);
        }
        ...
        $hasNotQ = $solver->textContains($allTexts, $Q, true);
        $hasNotS = $solver->textContains($allTexts, $S, true);
        $hasPorR = $solver->textContains($allTexts, $P, false) || $solver->textContains($allTexts, $R, false);
    ...
    'categorical_syllogism' => [
        ...
        if (empty($state['categoricals']) || count($state['categoricals']) < 2) {
            ...
            return $solver->solveInformalArgument($state);
        }
