...
return [
    ...
    'Relativistic' => [
        'meta' => ['base_case' => ['state' => 'Frame γ_{$n}', 'cond' => 'v_{$n} < c', 'outcome' => 'γ = 1/√(1−β²) ✅'], 'keywords' => '\b(lorentz|schwarzschild|relativity|time dilation|length contraction|spacetime|e=mc|mass.energy|tachyon|warp|ftl|faster.than.light|rest mass)\b', 'key' => 'special_relativity', 'name' => '🌌 Special & General Relativity (Synthetic)', 'branch_icon' => '🌌', 'academic_ref' => 'Einstein (1905, 1915), Lorentz (1904), Minkowski (1908)', 'trial' => 'We abstract spacetime intervals and Lorentz-invariant quantities.', 'deductive_axiom' => 'c is invariant in all inertial frames; massive objects cannot reach v = c.', 'inductive_limit' => 'Relativity is verified to extraordinary precision across cosmic scales.'],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
    ...
    'Electromagnetic' => [
        ...
        'phase1' => function(array &$state, string $thesis, $cas = null) {
                ...
                $state['proof_traces'][] = "- **Speed of Light Derivation**: c = 1/√(ε₀μ₀) = " . number_format(1 / sqrt(8.8541878128e-12 * 1.25663706212e-6), 0) . " m/s ✅";
    ...
    'Nuclear' => [
        'meta' => ['base_case' => ['state' => 'Nucleus A={$n}', 'cond' => 'N(t) = N₀e^(−λt)', 'outcome' => 'Decay ✅'], 'keywords' => '\b(radioactive|nuclear|fission|fusion|decay|half.life|bethe|binding energy|alpha|beta|gamma radiation|bohr|rutherford|photoelectric|nucleon|neutron|proton|uranium|electron volt)\b', 'key' => 'nuclear_physics', 'name' => '☢️ Nuclear Physics (Synthetic)', 'branch_icon' => '☢️', 'academic_ref' => 'Rutherford (1911), Bethe-Weizsäcker (1935), Fermi (1942)', 'trial' => 'We abstract nuclear decay and binding energy from the mass-energy equivalence.', 'deductive_axiom' => 'Nuclear reactions conserve baryon number, lepton number, and energy via E = mc².', 'inductive_limit' => 'Radioactive decay law is exponential and universally invariant.'],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
    ...
    'Chemical' => [
        'meta' => ['base_case' => ['state' => 'Reaction n={$n}', 'cond' => 'ΔG_{$n} < 0', 'outcome' => 'Spontaneous ✅'], 'keywords' => '\b(mass_out|mass_in|chemical|stoichiometr|lavoisier|reactants|products|arrhenius|gibbs|le chatelier|hess|acid|base|ph |buffer|equilibrium constant|combustion|synthesis|avogadro|molar)\b', 'key' => 'chemistry', 'name' => '⚗️ Stoichiometric Chemistry (Synthetic)', 'branch_icon' => '⚗️', 'academic_ref' => 'Lavoisier (1789), Gibbs (1876), Arrhenius (1889)', 'trial' => 'We abstract chemical transformation as algebraic permutation of atomic groupings.', 'deductive_axiom' => 'Conservation of mass (Lavoisier) holds absolutely in all chemical reactions within closed systems.', 'inductive_limit' => 'ΔG = ΔH − TΔS determines spontaneity universally for all chemical processes.'],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
                ...
                foreach ($temps as $T) {
                    $k = $A * exp(-$Ea / (8.314462618 * $T));
                    $state['trials'][] = [
                        'T (K)' => $T,
                        'T (°C)' => ($T - 273),
                        'k = A·e^(−Ea/RT)' => sprintf('%.3e', $k),
                        'Ratio k(T)/k(300K)' => number_format($k / ($A * exp(-$Ea / (8.314462618 * 300))), 2),
                        'Status' => 'Arrhenius ✅',
                    ];
                }
    ...
    'Biological' => [
        'meta' => ['base_case' => ['state' => 'Generation N={$n}', 'cond' => 'Allele Δf_{$n}', 'outcome' => 'Drift bounded ✅'], 'keywords' => '\\b(biology|organism|species|ecology|population|adaptation|phylogen)\\b', 'key' => 'evolutionary_biology', 'name' => '🌿 Evolutionary Biology (Synthetic)', 'branch_icon' => '🌿', 'academic_ref' => 'Darwin (1859), Hardy-Weinberg (1908), Mendel (1865)', 'trial' => 'We abstract population genetics and natural selection dynamics.', 'deductive_axiom' => 'Natural selection acts on heritable variation; Hardy-Weinberg equilibrium holds under ideal conditions.', 'inductive_limit' => 'Evolution by natural selection is confirmed by 4 billion years of fossil record and genomic data.'],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
                $state['proof_traces'][] = "\n**🌿 Vector Abstraction (Evolutionary Biology & Genetics)**:";
                $state['proof_traces'][] = "- **Hardy-Weinberg**: p² + 2pq + q² = 1  (allele frequency equilibrium under ideal conditions)";
                $state['proof_traces'][] = "- **Natural Selection**: Δp ≈ p·q·s  (s = selection coefficient; advantageous allele increases in frequency)";
                $state['proof_traces'][] = "- **Logistic Growth**: dN/dt = rN(1 − N/K);  N → K as t → ∞  (carrying capacity is an absolute ecological bound)";
                $state['proof_traces'][] = "- **Mendelian Genetics**: Aa × Aa → 1AA:2Aa:1aa (3:1 phenotype ratio, F2 generation)";
                $state['proof_traces'][] = "- **Central Dogma**: DNA → RNA → Protein (transcription, translation)";
                $state['proof_traces'][] = "- **Cellular Respiration**: C₆H₁₂O₆ + 6O₂ → 6CO₂ + 6H₂O + ~30-32 ATP";
                $pVals = [0.1, 0.3, 0.5, 0.7, 0.9];
                foreach ($pVals as $p) {
                    $q = 1 - $p;
                    $hw = $p * $p + 2 * $p * $q + $q * $q;
                    $state['trials'][] = [
                        'p (allele A)' => $p, 'q = 1−p' => round($q, 2),
                        'p² (AA)' => number_format($p * $p, 3), '2pq (Aa)' => number_format(2 * $p * $q, 3),
                        'q² (aa)' => number_format($q * $q, 3), 'H-W = 1?' => abs($hw - 1) < 1e-9 ? 'Yes ✅' : 'No ❌',
                    ];
                }
        },
        'phase2' => function(array &$state, string $thesis, $cas = null) {
            $domain = $state['domain'] ?? [];
            $isUnsolved = $state['is_unsolved'] ?? false;
                $state['symbolic_traces'][] = ['step' => '1', 'label' => 'Hardy-Weinberg Equilibrium', 'expr' => 'p + q = 1;  p² + 2pq + q² = 1  (binomial expansion) ✅'];
                $state['symbolic_traces'][] = ['step' => '2', 'label' => 'Natural Selection', 'expr' => 'Δp = p·q·s/(1 − s·q²)  (s > 0 → advantageous allele A increases)'];
                $state['symbolic_traces'][] = ['step' => '3', 'label' => 'Logistic Population Bound', 'expr' => 'dN/dt = rN(1 − N/K);  N → K as t → ∞  (absolute ecological ceiling)'];
                $state['symbolic_traces'][] = ['step' => '4', 'label' => 'Mendelian Inheritance', 'expr' => 'Aa × Aa: P(AA)=0.25, P(Aa)=0.50, P(aa)=0.25;  3:1 phenotype ratio'];
                $state['symbolic_traces'][] = ['step' => '5', 'label' => 'ATP Yield', 'expr' => 'Glycolysis(2ATP) + Krebs(2ATP) + OXPHOS(~28ATP) ≈ 30-32 ATP/glucose'];
                $state['proof_traces'][] = "Biological vectors are **consistent** with Hardy-Weinberg equilibrium and Darwinian evolution.";
        },
    ],
    'neuroscience' => [
        'meta' => ['keywords' => '\b(neuron|synapse|action potential|neurotransmitter|axon|dendrite|hodgkin|huxley|depolarisation|membrane potential)\b', 'key' => 'neuroscience', 'name' => '🧠 Neuroscience (Synthetic)', 'branch_icon' => '🧠', 'academic_ref' => 'Hodgkin-Huxley (1952), Nernst (1888)', 'trial' => 'We abstract action potentials as electrochemical wavefronts bounded by ion channel kinetics.', 'deductive_axiom' => 'Neural signal velocity is bounded by axon capacitance and resistance — always v ≪ c.', 'inductive_limit' => 'Hodgkin-Huxley equations accurately model all excitable cell membranes.'],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
    ...
    'Kinematic' => [
        'meta' => [
            'base_case' => ['state' => 'Dimension D={$n}', 'cond' => 'F_{$n} = m·a', 'outcome' => 'v < c ✅'],
            'keywords' => '\b(f\s*=\s*m|force|mass|acceleration|kinematic|newton|velocity|momentum|kinetic energy|potential energy|projectile|gravitational|torque|inertia)\b',
            'key' => 'classical_mechanics',
            'name' => '🍎 Classical Mechanics & Kinematics (Synthetic)',
            'branch_icon' => '🍎',
            'academic_ref' => 'Newton (1687), Galileo (1632)',
            'trial' => 'We abstract macroscopic deterministic motion.',
            'deductive_axiom' => 'F=ma dictates all macro-motion below c.',
            'inductive_limit' => 'Classical mechanics is empirically exact in the low-velocity macroscopic limit.'
        ],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
        ...
        'phase2' => function(array &$state, string $thesis, $cas = null) {
            ...
            $isUnsolved = $state['is_unsolved'] ?? false;
                ...
                if (isset($domain['equation'])) {
                    try {
                        $result = $cas->evaluateNumerically($domain['equation'], ['mass' => 10.0, 'acceleration' => 5.0]);
    ...
    'evolutionary_biology' => [
        'meta' => [
            'base_case' => ['state' => 'Generation N={$n}', 'cond' => 'Allele Δf_{$n}', 'outcome' => 'Drift bounded ✅'],
            'keywords' => '\bp\^2|2pq|hardy|weinberg|allele|genetic drift|speciation|evolv|darwin|natural selection|kin selection|fitness|mutation rate|reproductive\b',
            'key' => 'evolutionary_biology',
            'name' => '🌿 Evolutionary Biology (Synthetic)',
            'branch_icon' => '🌿',
            'academic_ref' => 'Darwin (1859), Hardy-Weinberg (1908)',
            'trial' => 'We abstract population genetics and natural selection dynamics.',
            'deductive_axiom' => 'Natural selection acts on heritable variation; Hardy-Weinberg equilibrium holds under ideal conditions.',
            'inductive_limit' => 'Evolution by natural selection is confirmed by 4 billion years of fossil record and genomic data.'
        ],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
    ...
    'genetics' => [
        'meta' => [
            'base_case' => ['state' => 'Generation N={$n}', 'cond' => 'Allele Δf_{$n}', 'outcome' => 'Drift bounded ✅'],
            'keywords' => '\b(dna|rna|chromosome|genotype|phenotype|allele|mutation|transcription|codon|protein|molecular biology)\b',
            'key' => 'genetics',
            'name' => '🧬 Genetics & Molecular Biology (Synthetic)',
            'branch_icon' => '🧬',
            'academic_ref' => 'Watson-Crick (1953), Mendel (1865)',
            'trial' => 'We abstract molecular replication and hereditary transfer.',
            'deductive_axiom' => 'DNA encodes RNA encodes proteins; mutations introduce random heritable variance.',
            'inductive_limit' => 'Genetic heredity is universal for all known Earth-based lifeforms.'
        ],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
    ...
    'cellular_biology' => [
        'meta' => [
            'base_case' => ['state' => 'Generation N={$n}', 'cond' => 'Allele Δf_{$n}', 'outcome' => 'Drift bounded ✅'],
            'keywords' => '\b(cell|atp|mitochondria|respiration|mitosis|meiosis|eukaryot|prokaryot|membrane|organelle)\b',
            'key' => 'cellular_biology',
            'name' => '🦠 Cellular Biology (Synthetic)',
            'branch_icon' => '🦠',
            'academic_ref' => 'Hooke (1665), Schwann (1839)',
            'trial' => 'We abstract cellular metabolism and compartmentalisation.',
            'deductive_axiom' => 'All living organisms are composed of cells; cells arise from pre-existing cells.',
            'inductive_limit' => 'Cell theory universally bounds biological organisation on Earth.'
        ],
        'phase1' => function(array &$state, string $thesis, $cas = null) {
